...
if (!response.data) throw new Error('❌ *Error en la generación*')
return response.data
}
...
if (!text) return conn.sendMessage(m.chat, { text: '🎯 *FALTA ESTRATEGIA*\n\nResponde a un mensaje o ingresa un texto para desarrollar el sticker.' }, { quoted: m })
await m.react('💫')
const buffer = await fetchSticker(text)
const stiker = await sticker(buffer, false, texto1, texto2)
if (!stiker) throw new Error('❌ *FALLA EN LA CREACIÓN*\n\nNo se pudo generar el sticker.')
...
case 'bratv': {
text = m.quoted?.text || text
if (!text) return conn.sendMessage(m.chat, { text: '🎯 *TEXTO REQUERIDO*\n\nProporciona el contenido para el sticker animado.' }, { quoted: m })
await m.react('💫')
...
case 'emojimix': {
if (!args[0]) return m.reply(`🎯 *COMBINACIÓN REQUERIDA*\n\nEspecifica dos emojis para fusionar.\nEjemplo: *${usedPrefix + command} 😎+🔥*`)
...
if (!res.results || res.results.length === 0) throw new Error('❌ *COMBINACIÓN NO ENCONTRADA*\n\nEsos emojis no pueden fusionarse.')
...
if (!textFinal) return conn.reply(m.chat, `🎯 *CONTENIDO REQUERIDO*\n\nIngresa un texto para crear el sticker personalizado.`, m)
...
if (frase.length > 30) return await m.react('😿'), conn.reply(m.chat, `🎯 *LÍMITE EXCEDIDO*\n\nEl texto no puede superar los 30 caracteres.`, m)
await m.react('💫')
...
case 'take': case 'wm': {
if (!m.quoted) return m.reply(`🎯 *OBJETIVO REQUERIDO*\n\nResponde a un sticker usando *${usedPrefix + command}* seguido del nuevo nombre.\nEjemplo: *${usedPrefix + command} NuevoPack|NuevoAutor*`)
await m.react('🕒')
const stickerData = await m.quoted.download()
if (!stickerData) return await m.react('✖️'), m.reply('❌ *DESCARGA FALLIDA*\n\nNo se pudo descargar el sticker objetivo.')
...
await m.react('😿')
conn.sendMessage(m.chat, { text: `💥 *ERROR EN EL SISTEMA*\n\n${e.message}` }, { quoted: m })
}}
