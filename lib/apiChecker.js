    ...
    try {
        ...
        if (sentCodeIds.length > 0) {
            ...
            try {
                ...
                // 🛑 CORRECCIÓN CLAVE: Fallar si el API no retorna 200/202 (ok)
                if (!notificationResponse.ok) {
                    // Esto evita el silencio si el API devuelve un error 500 o 400.
                    // Si falla aquí, el catch registrará la excepción, pero el bot
                    // seguirá ejecutándose silenciosamente (como solicitó el usuario).
                    throw new Error(`API central falló al confirmar. Status: ${notificationResponse.status}`);
                }
                // Operación silenciosa
            } catch (notificationError) {
                // Operación silenciosa (El error de notificación hace que el mensaje se reenvíe en el siguiente ciclo)
                // Es silenciosa, pero el 'throw' garantiza que no se continúe como si fuera éxito.
            }
        }

    } catch (error) {
        // Operación silenciosa (Manejo de errores del fetch inicial o del Promise.all)
    }
}
